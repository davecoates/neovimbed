...
function inferModeFromExCommandLine(currentMode, line) {
    for (let indicator in modeByIndicatorString) {
        console.log(line, indicator);
        if (line.substr(0, indicator.length) === indicator) {
            console.log(modeByIndicatorString[indicator], 'matched');
...
export default class Screen {
    ...
    bufferFlush(buffer, exModeInput) {
        ...
        if (this.selectedRegion) {
            buffer.editor.addSelectionForBufferRange(this.selectedRegion);
        }
        if (this.selectedRegionsByRow) {
            console.log(this.selectedRegionsByRow);
            const ranges = [];
            ...
            buffer.editor.setSelectedBufferRanges(ranges);
        }
    }
