...
import { getBufferContents, getBuffer } from './neovim-util';
...
export const $$textEditorBufferNumber = Symbol.for('Neovimbed/TextEditorBufferNumber');
...
export default class LegacyVirtualWindowManager {
    ...
    async onEnterBuffer(buffer:BufferDetails) : Promise {
        console.log('enter buffer', buffer);    
        const panes = atom.workspace.getPanes();
        for (const pane of panes) {
            for (const item of pane.getItems()) {
                if (atom.workspace.isTextEditor(item)) {
                    if (item[$$textEditorBufferNumber] === details.bufferNumber) {
        ...
        console.warn('Could not find matching text editor for buffer', buffer);
        // TODO: Find text editor and activate
    }
