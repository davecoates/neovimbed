...
export default class LegacyVirtualWindowManager {
    ...
    disableBufferTracking = new Map();
    pendingExternalBufferChanges = new Map();
    ...
    async onDidBufferStopChanging(editor:TextEditor) {
        ...
        // TODO: Some issues currently
        // If cursor sync is on we lose lines at end of file( !?)
        // If off we get extra lines (eg. );) repeated at end of file.
        const neovimBuffer = await getBuffer(this.client, editor[$$textEditorBufferNumber]);
        const lineSliceParams = [];
        for (const change of changes) {
            const { newRange, oldRange, newText } = change;
            const newLines = editor.getBuffer().lines.slice(newRange.start.row, newRange.end.row);
            lineSliceParams.push([oldRange.start.row, oldRange.end.row, true, true, newLines]);
        }
        await Promise.all(lineSliceParams.map(params => neovimBuffer.setLineSlice(...params)));
        // TODO: Now we have possibly added new lines which means rest of file
        // may be incorrect
        this.client.getCurrentWindow().then(win => {
            // As above - vim is 1 based for rows and 0 based for column
        ...
        let applyChanges = (textEditor) => {
            ...
            if (!Number.isNaN(lineNumber) && linesAdded > 0) {
                ...
                const p = getBuffer(this.client, statusLine.bufferNumber)
                    ...
                    .then(lines => buffer.insert([lineNumber + 1, 0], lines.join('\n')+'\n'));
                promises.push(p);
            }
            Promise.all(promises).then(() => {
                ...
                if (textEditor.getLineCount() > statusLine.totalLineCount) {
                    ...
                    getBuffer(this.client, statusLine.bufferNumber)
