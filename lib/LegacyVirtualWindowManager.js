...
export default class LegacyVirtualWindowManager {
    ...
    async onDidChangeCursorPosition(editor, cursor) {
        ...
        if (vimRow !== row || vimColumn !== column) {
            ...
            console.log('set vim cursor', vimRow, vimColumn, row, column, [row + 1, column]);
    ...
    async onDidBufferStopChanging(editor:TextEditor) {
        ...
        for (const change of changes) {
            ...
            await neovimBuffer.setLineSlice(oldRange.start.row, oldRange.end.row, true, true, newLines);
        }
    ...
    flush() {
        ...
        let applyChanges = (textEditor) => {
            ...
            const setTextInBufferRange = (range, text) => {
                ...
                const lineLength = buffer.lineLengthForRow(range[0][0]);
            ...
            if (textEditorLineCount < statusLine.totalLineCount) {
                ...
                getBuffer(this.client, statusLine.bufferNumber)
                    .then(neovimBuffer => neovimBuffer.getLineSlice(textEditorLineCount, statusLine.totalLineCount, true, true))
                    .then(lines =>
                          setTextInBufferRange(
                              [
